...
zcc.phone = {
	...
	showProgress: function (msg) {//显示加载等待框
		if(this.clientType == 'ANDROID'){
			...
			var url="qxflios::showProgress::"+msg; 
			execute(url);
		} else {
	...
	showShortMessage: function (msg) {//显示短消息
		if(this.clientType == 'ANDROID'){
			...
			var url="qxflios::showShortMessage::"+msg; 
			execute(url);
		} else {
	...
	showLongMessage: function (msg) {//显示短消息(指定时间长度)
		if(this.clientType == 'ANDROID'){
			...
			var url="qxflios::showMessage::"+msg; 
			execute(url);
		} else {
	...
	openWebKit: function (url, title, titleFlag, screenType, param, hasRightBtn, rightIcon, rightJs, hasRightBtn1, rightIcon1, rightJs1) {//打开webkit
		if(this.clientType == 'ANDROID'){
			...
			var url="qxflios::openWebKit::"+url+"::"+title+"::"+titleFlag+"::"+screenType+"::"+param+"::"+hasRightBtn+"::"+rightIcon+"::"+rightJs+"::"+hasRightBtn1+"::"+rightIcon1+"::"+rightJs1 
			execute(url);
		} else {
	...
	closeWebViewRef: function (className) {//关闭webkit并刷新父类
		if(this.clientType == 'ANDROID'){
			...
			var url="qxflios::closeWebViewRef::"+className; 
			execute(url)
		} else {
	...
	closeCallBack: function (jsStr) {//关闭webkit并执行父页面的js方法
		if(this.clientType == 'ANDROID'){
			...
			var url="qxflios::closeCallBack::"+jsStr; 
			execute(url);
		} else {
	...
	getLastLocation: function () {//获取地址经纬度json字符串
		if(this.clientType == 'ANDROID'){
			...
			xxx=''
			var url="qxflios::getLastLocation"; 
			execute(url)
			return xxx;
		} else {
	...
	getLocation: function (jsStr) {//获取地址经纬度json字符串
		if(this.clientType == 'ANDROID'){
			...
			var url="qxflios::getLocation::"+jsStr; 
			execute(url)
		} else {
	...
	getSysInfo: function (key) {//获取系统信息json字符串
		if(this.clientType == 'ANDROID'){
			...
			xxx=''
			var url="qxflios::getSysInfo::"+key; 
			execute(url)
			return xxx;
		} else {
	...
	getUserInfo: function (key) {//获取用户信息json字符串
		if(this.clientType == 'ANDROID'){
			...
			xxx='';
			var url="qxflios::getUserInfo::"+key; 
		    execute(url)
		    return xxx;
		} else {
	...
	showSelectDlg: function(data, textId, valueId, textKey, valueKey, hasSearch){//显示下拉框
		if(this.clientType == 'ANDROID'){
			...
			var url="qxflios::showSelectDlg::"+data.replace(/\'/gi,'\"')+"::"+textId+"::"+valueId+"::"+textKey+"::"+valueKey+"::"+hasSearch; 
		    execute(url)
		} else {
	...
	showDateDlg: function(type, domId, defaultValue){//显示日期框
		if(this.clientType == 'ANDROID'){
			...
			var url="qxflios::showDateDlg::"+type+"::"+domId+"::"+defaultValue; 
		    execute(url)
		} else {
	...
	openCamera: function(fileNameShowDom, filePathDom, fileIdDom, fileNameDom,jsScript){//打开照相机拍照
		if(this.clientType == 'ANDROID'){
			...
			var url="qxflios::openCamera::"+fileNameShowDom+"::"+filePathDom+"::"+fileIdDom+"::"+fileNameDom+"::"+jsScript; 
		    execute(url)
		} else {
	...
	openFileSelect: function(fileType, fileNameShowDom, filePathDom, fileIdDom, fileNameDom,jsScript){//打开照相机拍照
		if(this.clientType == 'ANDROID'){
			...
			var url="qxflios::openFileSelect::"+fileNameShowDom+"::"+filePathDom+"::"+fileIdDom+"::"+fileNameDom+"::"+fileType+"::"+jsScript; 
		    execute(url)
		} else {
    ...
    submitFileForm: function(formDomId, filePaths, fileIds, fileSavePath,moduleName,tableName, linkBsusinessId){//提交拍照文件表单
		if(this.clientType == 'ANDROID'){
			...
			var url="qxflios::submitFormWithMultiFiles::"+formDomId+"::"+filePaths+"::"+fileIds+"::"+fileSavePath+"::"+moduleName+"::"+tableName+"::"+linkBsusinessId; 
		    execute(url)
		} else {
	...
	previewServerPhoto: function(photoPath){//服务器图片预览
		if(this.clientType == 'ANDROID'){
			...
			var url="qxflios::previewServerPhoto::"+photoPath; 
		    execute(url)
		} else {
	...
	previewClientPhoto: function(photoPath){//本地图片预览
		if(this.clientType == 'ANDROID'){
			...
			var url="qxflios::previewClientPhoto::"+photoPath; 
		    execute(url)
		} else {
	...
	initCookie: function(){//COOKIE设置
		if(this.clientType == 'ANDROID'){
		    ...
		    document.cookie ='httpServerPort='+zcc.phone.getSysInfo('httpServerPort')+';path=/'
			document.cookie ='client_type=IOS;path=/';
			document.cookie ='QXFLBASE='+zcc.phone.getSysInfo('QXFLBASE')+';path=/';
			document.cookie ='mobileModel='+zcc.phone.getSysInfo('mobileModel')+';path=/';
			document.cookie ='versionName='+zcc.phone.getSysInfo('versionName')+';path=/';
			document.cookie ='versionIoS='+zcc.phone.getSysInfo('versionIoS')+';path=/';
			document.cookie ='ORG_ID='+zcc.phone.getSysInfo('ORG_ID')+';path=/';
			document.cookie ='POST_ID='+zcc.phone.getSysInfo('POST_ID')+';path=/'
	...
	photoFromAlbum: function(fileNameShowDom, filePathDom, fileIdDom, fileNameDom, mediaType){//选择相册图片
		if(this.clientType == 'ANDROID'){
			...
			var url="qxflios::photoFromAlbum::"+fileNameShowDom+"::"+filePathDom+"::"+fileIdDom+"::"+fileNameDom+"::"+mediaType; 
		    execute(url)
		} else {
	...
	showAlertSelDialog: function(data){//弹出对话框，里面有几项供点击选择
		if(this.clientType == 'ANDROID'){
			...
			var url="qxflios::showAlertSelDialog::"+data; 
		    execute(url)
		} else {
	...
	openVideo: function(url){//播放视频文件，包括远程地址和本地SD卡视频
		if(this.clientType == 'ANDROID'){
		    ...
		    var url="qxflios::openVideoFile::"+url; 
		    execute(url)
		} else {
	...
	putSettingValue:function(key,value){//数据库存放全局参数
	    if(this.clientType == 'ANDROID'){
		    ...
		    var url="qxflios::putSettingValue::"+key+"::"+value;
		    execute(url);
		} else {
	...
	getSettingValue:function(key,defaultValue){//数据库获取全局参数
	    if(this.clientType == 'ANDROID'){
		    ...
		    xxx='';
			var url="qxflios::getSettingValue::"+key+"::"+defaultValue; 
		    execute(url)
		    return xxx;
		} else {
			return "";
	...
	sms:function(msisdn,msg){//发送短信
	    if(this.clientType == 'ANDROID'){
		    ...
		    var url="qxflios::sms::"+msisdn+"::"+msg;
		    execute(url);
		} else {
	...
	showMakers:function(citylon,citylat,json,nav_flag){//地图打点
	    if(this.clientType == 'ANDROID'){
		    ...
		    var url="qxflios::showMakers::"+citylon+"::"+citylat+"::"+json+"::"+nav_flag;
		    execute(url);
		} else {
	...
	mapNavigation:function(startlon,startlat,endlon,endlat){//地图导航 开始经度，开始纬度 为空默认为当前人经纬度
	    if(this.clientType == 'ANDROID'){
		    ...
		    var url="qxflios::mapNavigation::"+startlon+"::"+startlat+"::"+endlon+"::"+endlat;
		    execute(url);
		} else {
...
var serverPath = zcc.phone.getSysInfo('serverPath');
...
$(function(){
	zcc.phone.initCookie();
   	$('.label-title').each(function(){
   		$(this).click(function(){
   		    var httpserverport = zcc.phone.getSysInfo('httpServerPort');
	...
	$('select').each(function(){
		...
		$('#'+selectSearchId).click(function(){
			if(eval($(this).attr('data')).length > 0){
				zcc.phone.showSelectDlg($(this).attr('data'), selectSearchId, selectId, optionLabel, optionValue, searchFlag);
			}
		});
