...
<body>
	<form id="form1" name="form1" action="${app}/sys/orgcfg/saveOrUpdateOrg.do" method="post">
		<div id="queryPanel" class="queryPanel">
		    ...
		    <div id="queryPanel_content" class="queryPanel_content">
				<table class="detail_table" cellspacing="0" cellpadding="0" style="width: 100%;">
					...
					<tr height="20">
						<td class="outDetail">
							<font color="red">*</font>&nbsp;受理电话：
						</td>
						<td class="outDetail2" colspan="3">
							<c:if test="${param.method != 'show'}">
								<input type="text" id="sl_phone" name="sl_phone" value="${map.sl_phone}" style="width: 90%"
									required="true" dataType="Require" msg="受理电话不能为空！" />
							</c:if>
							<c:if test="${param.method == 'show'}">
								${map.sl_phone}
							</c:if>	
					...
					<tr height="20">
						<td class="outDetail">
							<font color="red">*</font>&nbsp;检测部门名称：
						</td>
						<td class="outDetail2" colspan="3">
							<c:if test="${param.method != 'show'}">
								<input type="text" id="org_jcdw" name="org_jcdw" value="${map.org_jcdw}" style="width: 90%"
									required="true" dataType="Require" msg="检测部门名称不能为空！" />
							</c:if>
							<c:if test="${param.method == 'show'}">
								${map.org_jcdw}
							</c:if>	
					...
					<tr height="20">
						...
						<td class="outDetail2" colspan="3">
							<c:if test="${param.method != 'show'}">
								<input type="text" id="org_addr" name="org_addr" value="${map.org_addr}" style="width: 90%"
									required="true" dataType="Require" msg="地址不能为空！" />
							</c:if>
							<c:if test="${param.method == 'show'}">
								${map.org_addr}
							</c:if>	
					...
					<tr height="20">
						...
						<td class="outDetail2" colspan="3">
							<c:if test="${param.method != 'show'}">
								<input type="text" id="org_postal" name="org_postal" value="${map.org_postal}" style="width: 90%"
									required="true" dataType="Require" msg="邮编不能为空！" />
							</c:if>
							<c:if test="${param.method == 'show'}">
								${map.org_postal}
							</c:if>	
					...
					<tr height="20">
						...
						<td class="outDetail2" colspan="3">
							<c:if test="${param.method != 'show'}">
								<input type="text" id="org_phone" name="org_phone" value="${map.org_phone}" style="width: 90%"
									required="true" dataType="Require" msg="电话不能为空！" />
							</c:if>
							<c:if test="${param.method == 'show'}">
								${map.org_phone}
							</c:if>	
		...
		<br />
		<p align="center">
			<input type="hidden" id="org_level" name="org_level" value="${map.org_level}"  />
			<input type="hidden" id="region_id" name="region_id" value="${map.region_id}"  />
