...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		<title>屏蔽等电位及接地</title>
		<%@ include file="/inc/includeBase.jsp"%>
	</head>
	<body>
		<form id="form1" name="form1" method="post" action="${app}/cgjc/cgjc/pbddwList.do" target="listQuery">
			...
			<div id="queryPanel" class="queryPanel">
				...
				<div id="queryPanel_content" class="queryPanel_content"
					...
					<table class="search" cellspacing="0" cellpadding="0">
						<tr>
							<td class="outDetail" style="width: 50px;">
								建筑物：
							</td>
							<td class="outDetail2" style="width: 100px;">
								<input type="text" id="jzw" name="jzw" />
							</td>
						</tr>
					</table>
					<div id="queryPanel_footer" class="queryPanel_footer"
						...
						<c:if test="${flag == 1 &&  empty cflag}">
							...
							<zcc:button id="schbtn" cssClass="ovalbutton" onclick="plAdd();return false;">批量新增</zcc:button>
						</c:if>
					</div>
				</div>
			</div>
	...
	<script type="text/javascript">
		...
		sch();
  		...
  		function add() {
			MyWindow.OpenCenterWindowScroll('${app}/cgjc/cgjc/pbddwEdit.do?method=create&base_id='+'${base_id}','editPbddw',600,600);
		}
		
		function plAdd() {
			MyWindow.OpenCenterWindowScroll('${app}/cgjc/cgjc/pbddwMoreEdit.do?base_id='+'${base_id}','editPbddw',800,1000);
		}
