...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		<display:table name="list" requestURI="${app}/cgjc/cgjc/tkdybhqList.do"
			...
			<display:column title="类型" style="width:100px;" media="html">
				<c:choose>
					<c:when test="${row.type == 1 }">天馈电涌保护器</c:when>
					<c:when test="${row.type == 2 }">信号电涌保护器</c:when>
				</c:choose>
			</display:column>
			<display:column title="型号" property="xh" />
			<display:column title="安装数量" property="azsl"  />
			<display:column title="安装日期" property="azrq" />
			<display:column title="接地截面(mm²)" property="jdjm"  />
	...
	<script type="text/javascript">
		...
		function dqdybhqInfoEdit(id, method){
			MyWindow.OpenCenterWindowScroll('${app}/cgjc/cgjc/tkdybhqEdit.do?method='+method+'&id='+id,'editTkdybhq',550,800);
		}
		
		function copyDqdybhqInfo(base_id, id){
			MyWindow.OpenCenterWindowScroll('${app}/cgjc/cgjc/tkdybhqEdit.do?method=copy&base_id='+base_id+'&id='+id,'copyTkdybhq',550,800);
		}
		
		function del(id){//删除
			if(window.confirm('确定要删除吗？')){
				var res = new MyJqueryAjax('${app}/cgjc/cgjc/delTkdybhq.do','id='+id).request();
				if(res==1){
					alert('删除成功！');
