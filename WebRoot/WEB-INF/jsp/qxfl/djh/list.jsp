...
<html xmlns="http://www.w3.org/1999/xhtml">
		...
		<display:table name="list" requestURI="${app}/cgjc/cgjc/djhList.do"
			...
			<display:column title="操作" media="html" style="width:180px;">
				...
				<c:if test="${row.rec_id != user.operatorId }">
					<zcc:button cssClass="smallBtn_gray"
						onclick="tj('${row.id}');return false;">校核</zcc:button>&nbsp; 
					<zcc:button cssClass="smallBtn_gray"
	...
	<script type="text/javascript">
		...
		function tj(infoId){
			if(window.confirm('确定通过该报告？')){
				var res = new MyJqueryAjax('${app}/cgjc/cgjc/jhBg.do','id='+infoId).request();
				if(res==1){
					alert('校核成功！');
		...
		function th(infoId){
			if(window.confirm('确定退回该报告？')){
				MyWindow.OpenCenterWindowScroll('${app}/cgjc/cgjc/thEdit.do?id='+infoId+'&url='+'${app}/cgjc/cgjc/thBg.do'+'&flag='+0,'thwindow',400,600);
				/* var res = new MyJqueryAjax('${app}/cgjc/cgjc/thBg.do','id='+infoId).request();
				if(res==1){
					alert('退回成功！');
					window.parent.form1.submit();
				}else{
