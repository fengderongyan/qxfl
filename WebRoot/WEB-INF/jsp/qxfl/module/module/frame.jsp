	...
	<body>
		...
		<div id="tabsModuleRole" class="easyui-tabs" data-options="fit:true,border:false">
			<div id="easytab1" title="模块管理">
				<iframe id="queryModule" frameborder="0" height="99.5%" width="100%" scrolling="no"></iframe>
			</div>
			<div id="easytab2" title="角色权限管理" data-options="closable:false" style="overflow: auto;">
				<iframe id="queryRole" frameborder="0" height="99.5%" width="100%" scrolling="no"></iframe>
			</div>
			<div id="easytab3" title="模块权限管理" data-options="closable:false" style="overflow: auto;">
				<iframe id="queryPermit" frameborder="0" height="99.5%" width="100%" scrolling="no"></iframe>
			</div>
		</div>
			...
			onSelect:function(title){
				...
				if('easytab1'==tbid){
					if($('#tabsId').val() != 'easytab1'){
						$('#queryModule').attr('src', '${app}/module/queryModule.do?csrfToken='+$('#csrfToken').val());
						$('#tabsId').val('easytab1');
					}
				}else if('easytab2'==tbid){
					if($('#tabsId').val() != 'easytab2'){
						$('#queryRole').attr('src', '${app}/permit/roleQuery.do?csrfToken='+$('#csrfToken').val());
						$('#tabsId').val('easytab2');
					}
				}else if('easytab3'==tbid){
					if($('#tabsId').val() != 'easytab3'){
						$('#queryPermit').attr('src', '${app}/permit/queryModulePermit.do?csrfToken='+$('#csrfToken').val());
						$('#tabsId').val('easytab3');
					}
				}
			}
